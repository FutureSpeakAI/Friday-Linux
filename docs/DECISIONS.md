...
- **Deviation D-A21 (B5 regression, real crash reason confirmed via CI run
  33479726929's direct diagnostic evidence — commit a2adfd1's
  `image/scripts/boot-test-probe.sh`): `image/etc/friday/os.env`'s
  `FRIDAY_HOME` value corrected from `/home/friday` to
  `/home/friday/.friday` — the literal value SPEC.md §5 documents does not
  achieve the intent SPEC.md §5 itself states.** SPEC.md §5's exact
  sentence: "`FRIDAY_HOME` is `/home/friday`, so the app's default
  `~/.friday` layout is unchanged." The intent is unambiguous — the app's
  state should live somewhere equivalent to `~/.friday` — but the literal
  value given does not produce that once you account for how
  `friday_home()` actually works. Real PR-1 source
  (`agent_friday/paths.py`, checked directly against the `friday-desktop`
  checkout at tag `v5.9.0`):
  ```
  ...
  only appended in the *unset* fallback branch. So with the literal
  SPEC.md value `FRIDAY_HOME=/home/friday`, `cli.py`'s `SETUP_MARKER`
  resolved to `/home/friday/.setup_complete`, one directory level above
  where `image/firstboot/wizard.py`'s `seed_app_setup_marker()` actually
  writes it (`/home/friday/.friday/.setup_complete`, per SPEC.md §8.1's
  own Amendment-A1 text and this project's existing convention). Confirmed
  directly, not inferred, via the probe: `FRIDAY_HOME env = '/home/friday'`,
  `friday_home() = /home/friday` (no `.friday`), `cli.SETUP_MARKER =
  /home/friday/.setup_complete exists = False`, while
  `/home/friday/.friday/.setup_complete` genuinely existed on the real,
  correctly-mounted `@home` lockbox subvolume (`ls -la` showed it, `cat`
  showed `friday-linux-os-wizard`, `findmnt /home/friday` showed
  `/dev/mapper/friday-lockbox[/@home]` mounted `rw` at `/var/home/friday`
  — the mount-timing/shadowing theory considered before finding this was
  ruled out). This is why `_is_existing_user()` returned `False` and
  `cmd_start()` hit `Confirm.ask()` -> `EOFError` under systemd (no tty),
  crash-looping `friday.service` every `RestartSec=5` — a real regression
  from B5's v5.7.0 -> v5.9.0 repin (PR-1 landing changed `SETUP_MARKER`'s
  resolution from a literal `Path.home() / ".friday"` expression to this
  env-var-aware one; v5.7.0 had no `FRIDAY_HOME` concept at all, so
  `os.env`'s pre-existing `FRIDAY_HOME=/home/friday` line was inert and
  harmless until now).

  Per SPEC.md §18 rule 7 ("do the smallest thing that preserves the
  intent") rather than rule 2 (never override a DECIDED item without
  flagging it): this is a factual error in the spec's given value against
  the real, checked behavior of the code it configures, not a design
  choice being relitigated, so the one-line value correction is what
  SPEC.md's own stated intent requires. `os.env` now reads
  `FRIDAY_HOME=/home/friday/.friday`, matching `seed_app_setup_marker()`'s
  existing write path and restoring the "`~/.friday` layout is unchanged"
